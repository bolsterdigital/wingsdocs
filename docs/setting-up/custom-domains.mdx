...
First, decide between to options:
...
First, we will connect the 'root' domain (the one without 'www') to Wings. You will have to either edit your existing A record, or create a new one. Most domain name registrars will automatically assign an A record, so you probably have to edit the existing record.
